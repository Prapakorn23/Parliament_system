...
###Kunjira
from core.recommendation_engine import HybridRecommendationEngine
from core.trend_engine import TrendAnalysisEngine
app = Flask(__name__)
CORS(app)
...
analysis_data = {
    'detector': ThaiDuplicateWordDetector(
        ...
        enable_ngram=ENABLE_NGRAM_DETECTION,
        ngram_min=NGRAM_MIN_SIZE,
        ngram_max=NGRAM_MAX_SIZE,
        ngram_min_freq=NGRAM_MIN_FREQUENCY,
        enable_similarity=ENABLE_SIMILARITY_GROUPING,
        similarity_threshold=SIMILARITY_THRESHOLD,
        score_weights={
            'frequency': SCORE_WEIGHT_FREQUENCY,
            'spread': SCORE_WEIGHT_SPREAD,
            'length': SCORE_WEIGHT_LENGTH,
        },
    ),
...
def analyze_text():
    ...
    try:
        ...
        # สร้างรายงานคำซ้ำอย่างละเอียด
        duplicate_report = detector.get_duplicate_report(result)

        # แปลง similar_groups members ให้เป็น serializable
        similar_groups_json = {}
        for k, v in result.get('similar_groups', {}).items():
            similar_groups_json[k] = {
                'members': [{'word': w, 'frequency': f} for w, f in v['members']],
        ...
        return jsonify({
            ...
            'data': {
                ...
                # Enhanced duplicate detection data
                'duplicate_scores': result.get('duplicate_scores', {}),
                'similar_groups': similar_groups_json,
                'ngram_duplicates': result.get('ngram_duplicates', {}),
                'flagged_duplicates': result.get('flagged_duplicates', {}),
                'duplicate_count': result.get('duplicate_count', 0),
                'duplicate_threshold': result.get('duplicate_threshold', 2),
...
###Kunjira Start###
@app.route('/api/trend/category', methods=['GET'])
def trend_by_category():
    from core.trend_engine import TrendAnalysisEngine
    db = get_db_manager()

    engine = TrendAnalysisEngine(db)
    trend_raw = engine.trend_by_category(period="month")

    # รวมทุก category เข้าด้วยกันเป็น total ต่อเดือน
    combined = {}

    for cat, time_dict in trend_raw.items():
        for date, count in time_dict.items():
            combined[date] = combined.get(date, 0) + count

    # แปลงเป็น list format สำหรับ frontend
    trend_list = [
        {"date": date, "count": count}
        for date, count in sorted(combined.items())
    ]

    return jsonify({"trend": trend_list})
...
def recommend_documents():
    ...
    raw = engine.recommend(analysis_id)
    result = [{'analysis_id': aid, 'similarity': score} for aid, score in raw]
...
# ============ Recommendation Database API ============

RECOMMENDATION_UPLOAD_FOLDER = os.path.join(UPLOAD_FOLDER, 'recommendations')
os.makedirs(RECOMMENDATION_UPLOAD_FOLDER, exist_ok=True)


@app.route('/uploads/recommendations/<filename>')
def serve_recommendation_pdf(filename):
    """ให้บริการไฟล์ PDF ของ Recommendation"""
    return send_from_directory(RECOMMENDATION_UPLOAD_FOLDER, filename)


@app.route('/api/recommendations', methods=['GET'])
def list_recommendations():
    """ดึงรายการ recommendations ทั้งหมด"""
    try:
        db = get_db_manager()
...
def list_recommendation_categories():
    ...
    try:
        db = get_db_manager()
        cats = db.get_recommendation_categories()
        return jsonify({'success': True, 'data': cats})
    except Exception as e:
        return jsonify({'error': str(e)}), 500


@app.route('/api/recommendations/by-category/<category>', methods=['GET'])
def get_recommendations_by_category(category):
    """ดึง recommendations ตามหมวดหมู่"""
    try:
        db = get_db_manager()
        recs = db.get_recommendations_by_category(category)
        for r in recs:
            if r.get('created_at'):
                r['created_at'] = r['created_at'].isoformat()
            if r.get('updated_at'):
...
def create_recommendation():
    """สร้าง recommendation ใหม่ พร้อมอัปโหลด PDF"""
    try:
        ...
        if not title or not category:
            return jsonify({'error': 'กรุณากรอกชื่อวาระและหมวดหมู่'}), 400
        ...
        if 'pdf_file' in request.files:
            ...
            if pdf_file.filename:
                ...
                pdf_stored_name = f"{uuid.uuid4().hex}{ext}"
                filepath = os.path.join(RECOMMENDATION_UPLOAD_FOLDER, pdf_stored_name)
        ...
        return jsonify({'success': True, 'id': rec_id, 'message': 'บันทึกสำเร็จ'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500


@app.route('/api/recommendation/<int:rec_id>', methods=['PUT'])
def update_recommendation(rec_id):
    """แก้ไข recommendation"""
    try:
        title = request.form.get('title', '').strip()
        category = request.form.get('category', '').strip()
        description = request.form.get('description', '').strip()
        ...
        if 'pdf_file' in request.files:
            ...
            if pdf_file.filename:
                ...
                old_rec = db.get_recommendation(rec_id)
                if old_rec and old_rec.get('pdf_stored_name'):
                    old_path = os.path.join(RECOMMENDATION_UPLOAD_FOLDER, old_rec['pdf_stored_name'])
                    if os.path.exists(old_path):
                        os.remove(old_path)

                stored_name = f"{uuid.uuid4().hex}{ext}"
                filepath = os.path.join(RECOMMENDATION_UPLOAD_FOLDER, stored_name)
                pdf_file.save(filepath)

                update_data['pdf_filename'] = pdf_file.filename
                update_data['pdf_stored_name'] = stored_name
                update_data['pdf_size'] = os.path.getsize(filepath)

        success = db.update_recommendation(rec_id, update_data)
        if success:
            return jsonify({'success': True, 'message': 'อัปเดตสำเร็จ'})
        return jsonify({'error': 'ไม่พบข้อมูล'}), 404
    except Exception as e:
        return jsonify({'error': str(e)}), 500


@app.route('/api/recommendation/<int:rec_id>', methods=['DELETE'])
def delete_recommendation(rec_id):
    """ลบ recommendation"""
    try:
        db = get_db_manager()
        rec = db.get_recommendation(rec_id)
        if rec and rec.get('pdf_stored_name'):
            filepath = os.path.join(RECOMMENDATION_UPLOAD_FOLDER, rec['pdf_stored_name'])
            if os.path.exists(filepath):
                os.remove(filepath)
        success = db.delete_recommendation(rec_id)
        if success:
...
def reset_recommendations():
    """รีเซ็ตฐานข้อมูล Recommendation ทั้งหมด"""
    try:
        import glob
        for f in glob.glob(os.path.join(RECOMMENDATION_UPLOAD_FOLDER, '*.pdf')):
            try:
                os.remove(f)
            except:
                pass
        ...
        db = get_db_manager()
        db.reset_recommendations()
        return jsonify({'success': True, 'message': 'รีเซ็ตฐานข้อมูล Recommendation สำเร็จ'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500


@app.route('/api/recommendations/match', methods=['POST'])
def match_recommendations():
    """ค้นหา recommendations จากหมวดหมู่ที่พบมากที่สุดในการวิเคราะห์
    
    หากหมวดหมู่อันดับ 1 ไม่มีเอกสาร จะลองหมวดหมู่ถัดไปตามลำดับ total_frequency
    """
    try:
        data = request.get_json()
        ...
        if not category_summary:
            return jsonify({'success': True, 'data': [], 'matched_category': None})

        filtered = [c for c in category_summary if c['category'] != 'อื่นๆ']
        if not filtered:
            filtered = category_summary

        sorted_categories = sorted(filtered, key=lambda x: x['total_frequency'], reverse=True)

        db = get_db_manager()
        
        for cat_info in sorted_categories:
            matched_category = cat_info['category']
            recs = db.get_recommendations_by_category(matched_category)
            ...
            if recs:
                ...
                return jsonify({
                    ...
                    'top_frequency': cat_info['total_frequency'],
                    'top_category_info': {
        ...
        return jsonify({
            'success': True,
            'data': [],
            'matched_category': top_category['category'] if top_category else None,
            'top_frequency': top_category['total_frequency'] if top_category else 0,
            'top_category_info': {
                'category': top_category['category'],
                'unique_words': top_category['unique_words'],
                'total_frequency': top_category['total_frequency']
